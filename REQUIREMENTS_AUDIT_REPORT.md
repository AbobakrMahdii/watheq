...
| #     | Requirement                                | Status | Evidence                                                                                                                                                                                                                                                                                          |
| ----- | ------------------------------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4.2.1 | يحلّل الوثيقة باستخدام نموذج ذكاء اصطناعي  | ✅     | `ai/verify_document.py` → `verify_card()` uses **ElementClassifier** (EfficientNet-B0, binary genuine/forged) per visual element and **FontAnalyzer** (morphological text profiling) per text region. Trained models verify from learned features — no runtime reference comparison. Layout defined via `layout_config.yaml` per doc type. |
| 4.2.2 | يكشف علامات التلاعب أو التعديل             | ✅     | AI v3 pipeline: ElementClassifier detects forged elements (70% weight) + position/size ROI validation (30% weight). FontAnalyzer checks stroke width, char height, density, sharpness, histogram, and uniformity against learned font profiles. Detects missing elements, anomalous positioning, and font tampering. |
| 4.2.3 | يقيّم مستوى ثقة النتيجة (Confidence Score) | ✅     | ElementClassifier returns calibrated scores (0.0–1.0) per element. FontAnalyzer returns weighted font similarity scores. Overall confidence uses 85% pass threshold / 60% suspicious threshold. Critical elements enforce minimum 70% individual score. |
...
_Last updated: 2026-02-08 (AI v3 overhaul: ElementClassifier+FontAnalyzer replacing YOLOv8+Siamese, layout_config.yaml per doc type, 85% pass threshold, MultiChain-only blockchain, 8-stage pipeline, dashboard analytics with charts, data verification, admin notes)_
